...
gen_girgs(
    ...
    deg=[10, 15],
    dim=2,
    ple=[2.1, 2.25, 2.4, 2.55, 2.75],
    T=[0.0, 0.4, 0.53, 0.7, 0.76, 0.88, 0.9999],
    square=[False, True],
    seed=[64, 321, 356, 489, 836, 2323, 252, 34121],
)
...
gen_girgs(
    ...
    deg = [10, 15],
    dim=1,
    ple=[2.1, 2.25, 2.5, 2.75, 3.00, 3.35, 4.0],
    T=[0.0],
    square=[False],
    seed=[64, 321, 356, 489, 836, 2323, 252, 34121],
)
...
run.add(
    ...
    "code/release/chung_lu -n [[n]] --deg [[deg]] --ple [[ple]] [[file]]",
    {
        "n": 50000,
        "deg": [10, 20],
        "ple": [2.1, 2.25, 2.4, 2.55, 2.75, 3.00, 3.35, 3.9, 5.1, 25.0],
...
simple_experiment(
    ["deg_distr", "avg_deg_locality", "avg_detour_dist", "avg_dist", "size", "ecc_samples"],
    input_names_all,
)
...
run.add(
    ...
    {"algo": ["ifub_foursweephd", "ifub_hd", "random", "q1", "q2", "q3", "q4", "q5"], "input": input_names, "timeout": 1800},
...
run.add(
    "branching",
    "timeout [[timeout]] code/release/branching --algo [[algo]] --no-header input_data/adj_array/[[input]]",
    {"algo": ["ifub_hb", "ifub_foursweephd", "random"], "input": input_names, "timeout": 1800},
    allowed_return_codes=[0, 124, -15],
    stdout_file="output_data/branching/[[input]].csv",
    stdout_mod=lambda out, res: (
        out if res.returncode == 0 else "[[algo]],0,0,[[timeout]]"
    ),
    header_command="code/release/branching --only-header",
)
